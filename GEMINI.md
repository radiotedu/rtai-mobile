...
Terminal release refresh authorized by user: persistent local favorites (separate preferences file), accent-insensitive station search, session-only deadline-based sleep timer, keyboard guide and working T shortcut for Focus. Authentication/ERP/Gold contracts are unchanged. Thirty terminal tests include interactive key dispatch, preferences isolation and sleep expiry. Replace only terminal ZIP/TGZ plus checksums in v1.3.11, preserve Android binaries, and identify the newer terminal source commit in release notes.
