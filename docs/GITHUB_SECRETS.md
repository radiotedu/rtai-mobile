...
```powershell
$keystoreBase64 | gh secret set ANDROID_RELEASE_KEYSTORE_BASE64 --repo radiotedu/radiotedumobile
$keystorePassword | gh secret set ANDROID_KEYSTORE_PASSWORD --repo radiotedu/radiotedumobile
$keyAlias | gh secret set ANDROID_KEY_ALIAS --repo radiotedu/radiotedumobile
$keyPassword | gh secret set ANDROID_KEY_PASSWORD --repo radiotedu/radiotedumobile
```

After provisioning, confirm only the secret names and update timestamps with `gh secret list --repo radiotedu/radiotedumobile`. GitHub does not reveal stored secret values.
