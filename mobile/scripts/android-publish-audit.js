...
function main() {
  ...
  const checks = [
    ...
    check(/android:name="com\.android\.automotive"/.test(manifest), 'Native AAOS media opt-in is declared', 'com.android.automotive'),
    check(/android:appCategory="audio"/.test(manifest), 'AAOS app category is audio', 'android:appCategory=audio'),
    check(/android:name="androidx\.car\.app\.launchable"[\s\S]*?android:value="true"/.test(manifest), 'AAOS media service explicitly opts into launcher discovery', 'androidx.car.app.launchable=true'),
