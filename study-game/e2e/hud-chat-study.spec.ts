...
test('presents an immersive game HUD and room-scoped working chat', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Campus Shop' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Events' })).toBeVisible()
  const actionDockLayout = await page.locator('.action-dock').evaluate((dock) => {
    const dockBounds = dock.getBoundingClientRect()
    const stageBounds = document.querySelector('#study-game')!.getBoundingClientRect()
    const buttons = [...dock.querySelectorAll('button')].map((button) => button.getBoundingClientRect())
    return {
      count: buttons.length,
      gap: stageBounds.bottom - dockBounds.bottom,
  ...
  await expect(page.getByTestId('study-path-list')).toBeVisible()
  await expect(page.getByTestId('study-path-list').locator('.study-path-card')).toHaveCount(4)
  await expect(page.getByTestId('study-path-list')).toContainText('Daily Focus Goal')
  await page.getByRole('button', { name: 'Campus Events' }).click()
  await expect(page.getByTestId('event-list')).toBeVisible()
  await expect(page.getByTestId('study-path-list')).toBeHidden()
  await page.getByRole('button', { name: 'Close events' }).click()
  await expect(page.locator('.authority-chip')).toHaveAttribute('data-authority', 'verified')
  await expect(page.locator('html')).toHaveAttribute('data-study-authority', 'local')
  ...
  if (testInfo.project.name === 'desktop-chromium') {
    ...
    expect(stage!.width).toBeCloseTo(page.viewportSize()!.width, 0)
    expect(stage!.height).toBeCloseTo(page.viewportSize()!.height, 0)
...
test('turns sitting in the Library into a clear focus-session experience', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#study-mission-kicker')).toHaveText('VERIFIED FOCUS ACTIVE')
  await expect(page.locator('#study-mission-title')).toContainText('verified min to today')
  await expect(page.locator('#study-mission-reward')).toHaveText('UP TO 25 GOLD')
