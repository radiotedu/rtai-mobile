...
const forbiddenProductionFixturePatterns = [
  { name: 'local admin fixture account', pattern: /study-selin|study-mert|study-deniz|study-arda/ },
  { name: 'local admin fixture operator', pattern: /RadioTEDU Broadcast Manager/ },
]
...
for (const absolute of files) {
  ...
  if (relative.startsWith('dist/assets/admin-') && relative.endsWith('.js')) {
    for (const fixture of forbiddenProductionFixturePatterns) {
      if (fixture.pattern.test(content)) findings.push({ file: relative, issue: fixture.name })
    }
  }
}
