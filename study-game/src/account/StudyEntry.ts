...
export function resolveStudyEntry(
  ...
  const login = withReturnTo(sameOriginPath(config?.loginUrl, '/login/', location), returnTo)
  const register = withReturnTo(sameOriginPath(config?.registerUrl, '/register/', location), returnTo)
