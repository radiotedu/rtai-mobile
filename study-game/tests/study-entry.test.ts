...
describe('Study account entry', () => {
  it('builds same-origin account routes with a return target', () => {
    ...
    expect(entry.loginUrl).toBe('/login/?return_to=%2Fstudy%2F%3Froom%3Dlibrary')
    expect(entry.registerUrl).toBe('/register/?return_to=%2Fstudy%2F%3Froom%3Dlibrary')
  ...
  it('rejects cross-origin and credential-bearing entry links', () => {
    ...
    expect(entry.loginUrl).toMatch(/^\/login\//)
    expect(entry.registerUrl).toMatch(/^\/register\//)
    expect(entry.accountUrl).toBe('/account/')
