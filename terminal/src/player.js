...
function playerArguments(command, url, title) {
  return command.toLowerCase().includes('ffplay')
    ? ['-nodisp', '-vn', '-hide_banner', '-loglevel', 'error', '-volume', '80', url]
    : ['--no-video', '--force-window=no', '--input-terminal=yes', `--title=RadioTEDU - ${title}`, url];
}

class Player {
  constructor(configured) { this.process = null; this.command = findPlayer(configured); this.last = null; this.paused = false; }
  launch() {
    const child = spawn(this.command, playerArguments(this.command, this.last.url, this.last.title), {stdio: ['pipe', 'ignore', 'ignore']});
    this.process = child;
    this.paused = false;
